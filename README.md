...
- **左區**：`Layer=='Advisor'`（明確指定時強制生效）或 `Role` 含「顧問」「品質督導」。
...
- **左上角圖例**：僅列出本案 Excel 中**實際出現過**的徽章代碼（2 欄排列），未出現的預設徽章不會畫出，保持畫面乾淨。
- **連接線**：改為原生直角／垂直連接線 —— 同欄卡片（Top→SubTop、SubTop→Middle）以垂直直線相連；左／右欄至計畫經理採直角(elbow)連接線；計畫經理至各組別採「主幹＋水平匯流排＋垂直分支」樹狀直角連接。
- 版面高度（含每張卡片的動態高度、圖例、分組區塊、頁尾）皆為程式試算後動態決定，人員或徽章數量增加時投影片會自動加高，避免任何重疊。

## Layer 欄位防呆與徽章動態配色

- **Layer 大小寫防呆**：讀取 Excel 時自動去除頭尾空白並標準化大小寫（`top`/`TOP`/`Top` 皆視為 `Top`；`subtop` 視為 `SubTop`），避免使用者輸入小寫導致人員從組織圖消失。
- **徽章動態選色**：`BADGE_COLORS` 未收錄的徽章代碼不再統一顯示死板灰藍色，改以 MD5 雜湊從擴充色盤（`FALLBACK_BADGE_PALETTE`）中分配固定且具鑑別度的顏色，同一代碼在整份文件、每次執行皆保持同色。
- **圖例防呆**：`BADGE_MAP` 找不到全名時，圖例直接顯示代碼本身（如「綠」），不會因缺少對照而報錯。
