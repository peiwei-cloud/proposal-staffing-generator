...
REQUIRED_COLUMNS = [
    "Layer", "Role", "GroupName", "Name", "Title", "Company", "Badges", "Licenses", "PhotoName",
...
# 徽章代碼 -> 全名（水利工程標案標準用語，可依專案需求自行調整）
BADGE_MAP = {
    "技": "技師執照", "水": "水利技師", "土": "土木技師", "環": "環工技師",
    "碩": "碩士", "博": "博士", "品": "品管工程師", "安": "勞工安全衛生管理人員",
    "P": "PMP國際專案管理師", "採": "採購專業人員", "甲": "甲種職業安全衛生業務主管",
    "乙": "乙級廢棄物處理", "景": "國際景觀建築師協會會員",
}

# 水利工程標案標準色系
BADGE_COLORS = {
    "技": "E65100", "水": "0277BD", "土": "5D4037", "環": "2E7D32",
    "品": "00829B", "安": "C6538C", "P": "52616B", "甲": "D0C000",
    "景": "7A9A60", "碩": "2E7D32", "博": "6A1B9A", "採": "00838F",
    "乙": "455A64",
}
...
COLOR_CARD_BORDER = RGBColor(0x00, 0x00, 0x00)  # 卡片外框線統一改為純黑色（比照 Reference 簡報風格）
COLOR_TEXT_DARK = RGBColor(0x22, 0x22, 0x22)
COLOR_TEXT_GRAY = RGBColor(0x66, 0x66, 0x66)
COLOR_FOOTER_BG = RGBColor(0x5F, 0xA4, 0xBE)  # 頁尾統計列底色（比照 Reference 簡報：藍綠色 #5FA4BE）
COLOR_PLACEHOLDER = RGBColor(0xC9, 0xCF, 0xD8)

# 卡片標頭色彩分級（Top/Advisor/SubTop 橙色、Middle 深灰）與組別卡片灰階、主辦機關水藍
COLOR_LEADER_ORANGE = RGBColor(0xF2, 0x65, 0x22)   # Top / Advisor / SubTop 標頭
COLOR_GROUP_LEADER_HEADER = RGBColor(0x59, 0x59, 0x59)  # 組長卡片標頭
COLOR_GROUP_MEMBER_HEADER = RGBColor(0xA6, 0xA6, 0xA6)  # 組員卡片標頭
COLOR_ORG_BANNER_HEADER = RGBColor(0x00, 0xA3, 0xE0)    # 主辦機關標頭（水藍色）
COLOR_STAT_NUMBER = RGBColor(0x98, 0x48, 0x07)          # 頁尾醒目大數字（比照 Reference 簡報實際渲染色）
...
LAYER_CANONICAL_MAP = {
    ...
    "owner": "Owner", "supervisingauthority": "Owner", "client": "Owner",
}
...
def compute_footer_stats(df: pd.DataFrame) -> dict:
    """頁尾統計：碩士／博士人數統計採「Badges 代碼」與「Degree 文字」雙重掃描取聯集，
    避免使用者僅在 Degree 填寫學歷文字（如「土木工程研究所博士」）但未同步勾選 Badges 代碼時漏算，
    反之僅勾選 Badges 但 Degree 欄位空白時也能正確計入。
    技師人數改呼叫與 Summary 卡片共用的 count_pe_experts_and_pmp()，確保頁尾大數字與右上角 Summary
    卡片文字中的技師級專家人數永遠精準對齊（技師身份定義：Badges 含「技」或科別代碼 水/土/環/保/結，
    絕不含「景」國際景觀建築師協會會員）。"""
    avg_years = 0
    masters = 0
    doctors = 0
    ...
    try:
        ...
        avg_years = 0
    try:
        degree_master = df["Degree"].str.contains("碩", na=False)
        badge_master = df["Badges"].apply(lambda b: "碩" in parse_badges(b))
        masters = int((degree_master | badge_master).sum())
    except Exception:
        masters = 0
    try:
        degree_doc = df["Degree"].str.contains("博", na=False)
        badge_doc = df["Badges"].apply(lambda b: "博" in parse_badges(b))
        doctors = int((degree_doc | badge_doc).sum())
    except Exception:
        doctors = 0
    try:
        technicians, _pmp = count_pe_experts_and_pmp(df)
    except Exception:
        technicians = 0
    try:
...
BADGE_SORT_ORDER = ["技", "水", "土", "環", "碩", "博", "品", "安", "P", "採", "甲", "乙", "景"]
...
def parse_badges(badge_str: str):
    """拆解徽章代碼字串為獨立代碼清單。相容頓號「、」、全形逗號「，」、半形逗號「,」三種分隔符號，
    避免 Excel 以頓號分隔時（如「技、亞、P、安」）被誤判為單一未拆解的字串，導致圖例與晶片顯示亂碼。"""
    if not badge_str or str(badge_str).strip() in ("", "—", "-"):
        return []
    normalized = re.sub(r"[、，,]", ",", str(badge_str))
    badges = [b.strip() for b in normalized.split(",") if b.strip()]
    return sort_badges(badges)  # 全站統一排序，確保所有卡片／圖例／表格徽章順序一致


def _split_delim_list(text: str) -> list:
    """與 parse_badges 共用相同的分隔符號規則（頓號／全形逗號／半形逗號），但不排序、不視為徽章代碼，
    供 Licenses 欄位等純文字清單拆解使用。"""
...
def build_dynamic_badge_map(df: pd.DataFrame) -> dict:
    """若 Excel 提供 Licenses 欄位，且與 Badges 欄位拆解後的證照代碼數量一致，逐一對應建立
    「代碼 -> 證照全名」動態對照表，優先於程式內建的靜態 BADGE_MAP（供圖例與人員卡片查詢全名時使用）。
    數量不一致或 Licenses 留空時，該筆資料略過，不影響其餘人員的動態對照。
    ⚠️ 配對時刻意使用 _split_delim_list()（保留原始欄位順序）而非 parse_badges()（會依全域規則重新排序），
    確保「第 N 個代碼」精準對應「第 N 個證照全名」，不因排序而錯位配對。"""
    dynamic_map = {}
    if "Licenses" not in df.columns:
        return dynamic_map
    for _, row in df.iterrows():
        codes = _split_delim_list(row.get("Badges", ""))
        names = _split_delim_list(row.get("Licenses", ""))
        if codes and names and len(codes) == len(names):
            for code, name in zip(codes, names):
                if code and name and code not in dynamic_map:
                    dynamic_map[code] = name
    return dynamic_map


def badge_full_name(code: str, dynamic_map: dict = None) -> str:
    """徽章代碼查全名：優先採用 Excel Licenses 提供的動態對照，其次靜態 BADGE_MAP，
    皆找不到則直接回傳代碼本身，避免 KeyError 或圖例顯示空白。"""
    if dynamic_map and code in dynamic_map:
        return dynamic_map[code]
    return BADGE_MAP.get(code, code)
...
def _add_photo_or_placeholder(slide, x, y, size, photo_bytes):
    ...
    _set_shape_fill(ph, COLOR_PLACEHOLDER, line_color=COLOR_CARD_BORDER, line_width=Pt(0.75))
...
def _add_badge_row(slide, x, y, badges, chip_w=Inches(0.22), chip_h=Inches(0.22), gap=Inches(0.04)):
    cx = x
    for b in badges:
        color = RGBColor.from_string(get_badge_color(b))
        chip = slide.shapes.add_shape(MSO_SHAPE.RECTANGLE, cx, y, chip_w, chip_h)
...
def _add_legend(slide, x, y, used_badges, dynamic_map: dict = None):
    """繪製左上角圖例：僅列出本案實際出現的徽章，2 欄排列，一列一張證照、色塊沿用 get_badge_color() 動態配色；
    全名優先查詢 Excel Licenses 動態對照，其次靜態 BADGE_MAP，找不到則直接顯示代碼本身，避免 KeyError。"""
    if not used_badges:
        return
    for i, code in enumerate(used_badges):
        ...
        chip = slide.shapes.add_shape(MSO_SHAPE.RECTANGLE, cx, cy, _LEGEND_ICON, _LEGEND_ICON)
        _set_shape_fill(chip, color)
        chip.text_frame.word_wrap = False
        chip.text_frame.vertical_anchor = MSO_ANCHOR.MIDDLE
        chip.text_frame.margin_left = 0
        ...
        label = badge_full_name(code, dynamic_map)  # 圖例找不到全名時，直接顯示代碼本身，避免報錯
...
_SUMMARY_CARD_H = Inches(1.15)
_BANNER_GAP_BASE = Inches(0.1)
_TOP_ROW_FINAL_GAP_BASE = Inches(0.18)


def compute_fixed_top_row_height(legend_h: Emu) -> Emu:
    """圖例／Summary 卡片列本身高度：不隨三層安全網縮放（皆為內容驅動的小型元件）"""
    return max(legend_h, _SUMMARY_CARD_H)


def compute_banner_unit_height(host_org: str, client_org: str, scale: float = 1.0, extra_lines: int = 0) -> Emu:
    """機關卡片（含其下方間距）所需高度，隨 scale 等比例縮放，與領導卡片同步壓縮，
    確保三層安全網觸發時機關卡片與領導卡片仍維持相同尺寸，且不會因固定預留過大而導致下方內容溢出。
    extra_lines 與領導卡片共用同一值，確保兩者尺寸永遠一致（即使領導卡片因長職稱/公司名而加高）。"""
    if not (host_org or client_org):
        return Inches(0)
    return int(_leader_card_height(scale, extra_lines) + _BANNER_GAP_BASE * scale)


def _add_compact_org_banner(slide, slide_w, host_org: str, client_org: str, card_w: Emu, card_h: Emu):
    """主辦／委辦機關卡片：水藍色標頭條（顯示「主辦機關」/「委辦機關」）+ 白底卡片顯示機關名稱。
    寬度／高度強制與 Top/Advisor/SubTop 領導卡片完全一致（card_w/card_h 由呼叫端傳入），確保頂層視覺統一。"""
    if not (host_org or client_org):
        return
    items = []
    if host_org:
        items.append(("主辦機關", host_org))
    if client_org:
        items.append(("委辦機關", client_org))
    gap = Inches(0.3)
    total_w = card_w * len(items) + gap * (len(items) - 1)
    start_x = int((slide_w - total_w) / 2)
    header_h = _LC_HEADER_H
    body_h = card_h - header_h

    for i, (label, val) in enumerate(items):
        bx = start_x + i * (card_w + gap)
        # 白底卡片主體
        card = slide.shapes.add_shape(MSO_SHAPE.RECTANGLE, bx, 0, card_w, card_h)
        _set_shape_fill(card, COLOR_CARD_BG, line_color=COLOR_CARD_BORDER, line_width=Pt(1))
        # 水藍色標頭
        header = slide.shapes.add_shape(MSO_SHAPE.RECTANGLE, bx, 0, card_w, header_h)
        _set_shape_fill(header, COLOR_ORG_BANNER_HEADER)
        ...
        _set_pptx_run_ea_font(run)
        # 機關名稱（黑字 12pt 粗體，置中）
        _add_textbox(slide, bx, header_h, card_w, body_h, val, size=12, bold=True,
...
# Summary 卡片統計分類：具備任一徽章即計入該類別（同一人可能同時被兩類計入，不互斥）
SUMMARY_PE_BADGES = {"水", "土", "環", "保", "結", "技"}
SUMMARY_PMP_BADGES = {"P", "品", "安", "採"}
# 技師「科別」徽章（不含通用「技師執照」代碼「技」），用於 Summary「技師級專家」統計；
# 不含「景」(國際景觀建築師協會會員)——該資格不計入技師級專家人數統計。
# 未來新增科別徽章代碼（如結構技師、大地技師）只需擴充 BADGE_MAP／此集合即可自動納入統計
SUMMARY_PE_SPECIALTY_BADGES = SUMMARY_PE_BADGES - {"技"}
# 人員卡片「職稱稱謂」顯示專用集合：額外納入「景」，因為國際景觀建築師協會會員仍是合理的職稱稱謂，
# 只是不應計入 Summary／頁尾的「技師級專家」統計數字（兩者用途不同，刻意分開定義）
TITLE_SUFFIX_SPECIALTY_BADGES = SUMMARY_PE_SPECIALTY_BADGES | {"景"}


def get_technician_suffix(row, dynamic_map: dict = None) -> str:
    """判斷該人員是否具備技師（或同等專業資格）身份，回傳應附加於職稱後的稱謂全名（如「水利技師」「國際景觀建築師協會會員」）；
    無相關身份則回傳空字串，不可硬加無謂稱謂。此處使用 TITLE_SUFFIX_SPECIALTY_BADGES（含「景」），
    與 Summary／頁尾「技師級專家」統計刻意使用的 SUMMARY_PE_SPECIALTY_BADGES（不含「景」）用途不同、互不影響。
    判斷優先序：① Badges 含具體科別代碼（水/土/環/景…）→ 該科別全名；
    ② Badges 含通用「技」代碼 → 「技師」；
    ③ Title／Expertise 文字提及任一科別全名 → 該科別全名（CV 文字補強）。"""
    badges = set(parse_badges(row.get("Badges", "")))
    for code in sort_badges(list(badges & TITLE_SUFFIX_SPECIALTY_BADGES)):
        return badge_full_name(code, dynamic_map)
    if "技" in badges:
        return "技師"
    text_blob = f"{row.get('Title','') or ''}{row.get('Expertise','') or ''}"
    for code in sort_badges(list(TITLE_SUFFIX_SPECIALTY_BADGES)):
        name = badge_full_name(code, dynamic_map)
        # 僅當代碼有「真正的多字元全名」（而非退回顯示代碼本身）時才進行文字比對，
        # 避免代碼未定義全名時，其單一漢字巧合出現在敘述文字中（如「保」出現在「水土保持」）造成誤判
        if name and name != code and len(name) > 1 and name in text_blob:
            return name
    return ""


def format_person_title(row, dynamic_map: dict = None) -> str:
    """組合人員卡片顯示用職稱文字：【職稱 + 技師稱謂】（如「資深協理 水利技師」）；
    若職稱本身已包含該稱謂文字則不重複附加；無技師身份則僅顯示原職稱，不多加無謂稱謂。"""
    title = safe_text(row.get("Title", ""), "")
    suffix = get_technician_suffix(row, dynamic_map)
    if suffix and suffix not in title:
        return f"{title} {suffix}".strip()
    return title


def get_effective_badges(row, dynamic_map: dict = None) -> set:
    """回傳該人員的『有效徽章集合』：Excel Badges 欄位 + 從 CV 相關文字欄位（Title/Expertise/BioNarrative/Degree）
    掃描技師科別全名補強。修復僅在人員簡介／經歷提及技師科別（如「水利技師」），但 Excel Badges 欄位
    未同步勾選對應代碼時，Summary 統計漏算該人員科別的問題。"""
    explicit = set(parse_badges(row.get("Badges", "")))
    text_blob = "".join([
        str(row.get("Title", "") or ""), str(row.get("Expertise", "") or ""),
        str(row.get("BioNarrative", "") or ""), str(row.get("Degree", "") or ""),
    ])
    inferred = set()
    for code in SUMMARY_PE_SPECIALTY_BADGES:
        full_name = badge_full_name(code, dynamic_map)
        # 同上：僅當代碼有真正的多字元全名時才比對，避免單一漢字誤判
        if full_name and full_name != code and len(full_name) > 1 and full_name in text_blob:
            inferred.add(code)
    return explicit | inferred


def count_pe_experts_and_pmp(df: pd.DataFrame, dynamic_map: dict = None) -> tuple:
    """單一權威計算函式：技師級專家人數（Badges 含「技」或科別代碼 水/土/環/保/結，絕不含「景」）
    與 PMP/品管/安衛專業人才人數。頁尾統計列的「技師」欄位與右上角 Summary 卡片的「技師級專家」文字
    皆呼叫本函式取得數字，確保兩處永遠精準對齊、不會各自為政產生落差。"""
    if dynamic_map is None:
        dynamic_map = build_dynamic_badge_map(df)
    effective_badges_list = [get_effective_badges(r, dynamic_map) for _, r in df.iterrows()]
    pe_experts = sum(1 for b in effective_badges_list if b & SUMMARY_PE_BADGES)
    pmp_managers = sum(1 for b in effective_badges_list if b & SUMMARY_PMP_BADGES)
    return pe_experts, pmp_managers


def build_summary_stats(df: pd.DataFrame, expertise_override: str = "") -> dict:
    """團隊學經歷 Summary 卡片所需統計數據：
    - total：總團隊人數
    - pe_experts：具備【水/土/環/保/結/景/技】任一徽章（含 CV 文字補強判斷）之總人數（技師級專家，不重複計人）
    - pe_breakdown：技師科別分項統計文字，如「水利技師 3 人、土木技師 2 人」（依全域徽章排序、僅列出實際出現的科別）
    - pmp_managers：具備【P/品/安/採】任一徽章之總人數（PMP/品管/安衛專業人才）
    - expertise：專業涵蓋領域文字（自動萃取或手動覆寫）
    """
    total = len(df)
    dynamic_map = build_dynamic_badge_map(df)
    ...
    pe_experts, pmp_managers = count_pe_experts_and_pmp(df, dynamic_map)

    # 分項科別統計：僅統計具體科別徽章（不含通用「技」），依全域排序規範排列，只列出實際出現的科別
    specialty_counts = {}
    for badges in effective_badges_list:
        for code in (badges & SUMMARY_PE_SPECIALTY_BADGES):
            specialty_counts[code] = specialty_counts.get(code, 0) + 1
    breakdown_parts = [
        f"{badge_full_name(code, dynamic_map)} {specialty_counts[code]} 人"
        for code in sort_badges(list(specialty_counts.keys()))
    ]
    pe_breakdown = "、".join(breakdown_parts)
    ...
    return {"total": total, "pe_experts": pe_experts, "pe_breakdown": pe_breakdown,
            "pmp_managers": pmp_managers, "expertise": expertise_str}
...
def _add_summary_card(slide, x, y, w, summary: dict):
    ...
    _set_shape_fill(card, COLOR_CARD_BG, line_color=COLOR_CARD_BORDER, line_width=Pt(1.25))

    pad = Inches(0.09)
    blue_w = Inches(1.3)
    blue_box = slide.shapes.add_shape(MSO_SHAPE.RECTANGLE, x + pad, y + pad, blue_w - pad, h - pad * 2)
    ...
    pe_part = f"{summary['pe_experts']} 位技師級專家"
    if summary.get("pe_breakdown"):
        pe_part += f"（{summary['pe_breakdown']}）"
    line1 = f"配置 {pe_part}及 {summary['pmp_managers']} 位 PMP/品管/安衛專業人才，具備專業執照與高階碩博士學歷"
    line2 = f"專業涵蓋：{summary['expertise']}"
    _add_textbox(slide, text_x, y + Inches(0.1), text_w, Inches(0.62), line1,
                 size=9.5, bold=False, color=COLOR_TEXT_DARK, align=PP_ALIGN.LEFT, wrap=True)
    _add_textbox(slide, text_x, y + Inches(0.78), text_w, Inches(0.34), line2,
...
def get_owner_rows(df: pd.DataFrame) -> list:
    """依 Layer=='Owner'（含 SupervisingAuthority／Client 別名，讀取時已標準化）篩選主辦/委辦機關資料。
    Role 作為機關別（如「主辦機關」「委辦機關」），Name 作為機關名稱。"""
    return df[df["Layer"] == "Owner"].to_dict("records")


def resolve_org_banner_info(df: pd.DataFrame, host_org_ui: str, client_org_ui: str):
    """機關名稱彈性解析：主辦機關／委辦機關各自獨立判斷——UI 側邊欄該欄位有填寫則優先採用；
    該欄位留空則自動讀取 Excel 中 Layer=='Owner' 的資料補上（Role 含「委辦」者視為委辦機關，
    其餘視為主辦機關），避免使用者忘記於側邊欄填寫時，組織圖頂端機關框莫名消失。"""
    host_org = (host_org_ui or "").strip()
    client_org = (client_org_ui or "").strip()
    if not host_org or not client_org:
        excel_host, excel_client = "", ""
        for r in get_owner_rows(df):
            role = safe_text(r.get("Role", ""), "").strip()
            name = safe_text(r.get("Name", ""), "").strip()
            if not name:
                continue
            if "委辦" in role:
                if not excel_client:
        ...
        if not host_org:
            host_org = excel_host
        if not client_org:
            client_org = excel_client
    return host_org, client_org


# 協力廠商卡片配色（方案 B：淺色系莫蘭迪風）—— 標頭深藍灰、內容區極淺天藍灰、純白卡片底圖
COLOR_PARTNER_HEADER = COLOR_NAVY  # 與領導卡片職稱條同色，維持整體色系一致
...
_LC_TITLE_LINE_H_UNIT = Inches(0.135)  # 職稱區單行文字大約所需高度（含 line_spacing 1.1 的餘裕）


def _pick_title_font_size(text: str) -> float:
    """依職稱＋公司文字長度動態選擇字級：超過 25 字降至 8pt，超過 15 字降至 8.5pt，否則維持 9pt"""
    n = len(text or "")
    if n > 25:
        return 8.0
    if n > 15:
...
def _estimate_title_lines(text: str, info_w_in: float) -> tuple:
    """粗略估算職稱文字在給定寬度(英吋)下、以動態選定字級呈現時所需的行數。
    中文字近似正方形，字元寬度約略等於字級(pt)換算英吋再乘上安全係數。
    回傳 (建議字級, 預估所需行數)。"""
    if not text:
        return 9.0, 1
    font_pt = _pick_title_font_size(text)
    char_w_in = font_pt * 1.05 / 72
    chars_per_line = max(1, int(max(info_w_in, 0.3) / char_w_in))
    lines = max(1, math.ceil(len(text) / chars_per_line))
    return font_pt, lines


def _leader_card_title_text(row, dynamic_map: dict = None) -> str:
    """組合領導卡片職稱區顯示文字：【職稱＋技師稱謂】｜【公司名稱】"""
    company = safe_text(row.get("Company", ""), "")
    title_with_suffix = format_person_title(row, dynamic_map)
    return clean_separator_join([title_with_suffix, company], sep="｜")


def _leader_card_extra_title_lines(row, card_w: Emu, dynamic_map: dict = None) -> int:
    """估算該人員職稱區需要的『額外行數』（超過 1 行的部分），供上層統一決定整欄卡片的動態加高幅度，
    確保長職稱／長公司名稱（如「美華集團 技術經理 -美華環境科技股份有限公司」）100% 完整呈現，不溢出、不與徽章重疊。"""
    info_x_offset = _LC_PHOTO_LEFT_PAD + _LC_PHOTO_SIZE + _LC_INFO_GAP
    info_w_in = (card_w - info_x_offset - _LC_RIGHT_PAD) / 914400
    text = _leader_card_title_text(row, dynamic_map)
    _, lines = _estimate_title_lines(text, info_w_in)
    return max(0, lines - 1)


def _leader_card_height(scale: float = 1.0, extra_lines: int = 0) -> Emu:
    """卡片高度：基礎（照片驅動）高度 + 職稱區因長文字換行所需的額外高度（隨 scale 等比例縮放，
    確保三層安全網觸發縮放時，額外預留的換行空間也同步等比例壓縮)。"""
    base = (_LC_HEADER_H + _LC_TOP_PAD + _LC_PHOTO_SIZE + _LC_BOTTOM_PAD) * scale
    extra = _LC_TITLE_LINE_H_UNIT * extra_lines * scale
    return int(base + extra)


def _leader_card_header_color(row) -> RGBColor:
    """依 Layer 決定領導卡片標頭底色：Middle=深灰，其餘（Top/Advisor/SubTop）=橙色"""
    if row.get("Layer", "") == "Middle":
        return COLOR_HEADER_GRAY
    return COLOR_LEADER_ORANGE


def _add_leader_card(slide, cx, top, row, photos, card_w=Inches(2.8), scale: float = 1.0,
                      dynamic_map: dict = None, extra_lines: int = 0):
    """橫向領導卡片：頂部職稱標頭列（依 Layer 分級配色），下方左側照片、右側姓名/職稱(含技師稱謂)與公司/徽章由上至下排列。
    extra_lines：因職稱/公司文字過長需要換行時，由上層統一計算的額外行數，用於動態加高卡片並同步下移徽章列，
    確保長文字 100% 完整呈現、不溢出卡片、不與徽章晶片重疊。"""
    header_h = int(_LC_HEADER_H * scale)
    ...
    extra_h = int(_LC_TITLE_LINE_H_UNIT * extra_lines * scale)
    card_h = header_h + top_pad + photo_size + bottom_pad + extra_h
    ...
    # 職稱標頭：Top/Advisor/SubTop 橙色、Middle 深灰
    header = slide.shapes.add_shape(MSO_SHAPE.RECTANGLE, left, top, card_w, header_h)
    _set_shape_fill(header, _leader_card_header_color(row))
    header.text_frame.word_wrap = True
    header.text_frame.vertical_anchor = MSO_ANCHOR.MIDDLE
    p = header.text_frame.paragraphs[0]
    p.alignment = PP_ALIGN.CENTER
    ...
    title_h = int(photo_size * 0.24) + extra_h  # 職稱區隨 extra_lines 動態加高，容納換行文字
    ...
    title_text = _leader_card_title_text(row, dynamic_map)
    title_font_pt, _ = _estimate_title_lines(title_text, info_w / 914400)
    title_y = name_y + name_h
    _add_textbox(slide, info_x, title_y, info_w, title_h, title_text,
                 size=title_font_pt, color=COLOR_TEXT_GRAY, align=PP_ALIGN.LEFT, line_spacing=1.1)

    badges = parse_badges(row["Badges"])
    if badges:
        badge_y = title_y + title_h
        chip_size = min(Inches(0.22), badge_h)
        _add_badge_row(slide, info_x, badge_y, badges, chip_w=chip_size, chip_h=chip_size, gap=Inches(0.04))

    return left, top, card_w, card_h


# 組名標頭條
_GBH_NAME_H = Inches(0.26)
_GBH_GAP_AFTER_NAME = Inches(0.07)

# 組長獨立卡片
_GBH_LEADER_HEADER_H = Inches(0.2)
_GBH_LEADER_PHOTO = Inches(0.5)
_GBH_LEADER_TOP_PAD = Inches(0.05)
_GBH_LEADER_BOTTOM_PAD = Inches(0.05)
_GBH_GAP_AFTER_LEADER = Inches(0.07)

# 組員獨立卡片
_GBH_MEMBER_HEADER_H = Inches(0.18)
_GBH_MEMBER_LINE_H = Inches(0.2)
_GBH_MEMBER_TOP_PAD = Inches(0.05)
_GBH_MEMBER_BOTTOM_PAD = Inches(0.06)

_GB_MIN_H = Inches(1.4)


def _group_leader_title_extra_lines(leader_row, block_w: Emu, scale: float, dynamic_map: dict = None) -> int:
    """估算組長卡片職稱區（含技師稱謂）在該組別欄寬下需要的『額外行數』，
    與 _leader_card_extra_title_lines 相同邏輯，確保長職稱（如「資深技術副理兼任專案協調人 國際景觀建築師協會會員」）
    100% 完整呈現、不溢出組長卡片邊界、不與下方組員卡片重疊。"""
    if leader_row is None:
        return 0
    photo_size = int(_GBH_LEADER_PHOTO * scale)
    info_x_offset = int(Inches(0.1) * scale) + photo_size + int(Inches(0.08) * scale)
    info_w_in = max((block_w - info_x_offset - int(Inches(0.08) * scale)) / 914400, 0.3)
    text = format_person_title(leader_row, dynamic_map)
    _, lines = _estimate_title_lines(text, info_w_in)
    return max(0, lines - 1)


def _group_block_height(n_members: int, scale: float = 1.0, extra_lines: int = 0) -> Emu:
    """與 _add_group_block 實際繪製流程逐段對應的高度公式（三張獨立卡片：組名條＋組長卡＋組員卡），
    同一組具名常數，避免試算與繪製結果不一致。extra_lines：組長職稱因換行所需的額外行數。"""
    leader_extra = _LC_TITLE_LINE_H_UNIT * extra_lines
    leader_card_h = _GBH_LEADER_HEADER_H + _GBH_LEADER_TOP_PAD + _GBH_LEADER_PHOTO + _GBH_LEADER_BOTTOM_PAD + leader_extra
    member_card_h = (_GBH_MEMBER_HEADER_H + _GBH_MEMBER_TOP_PAD
                      + _GBH_MEMBER_LINE_H * max(n_members, 1) + _GBH_MEMBER_BOTTOM_PAD)
    h = (_GBH_NAME_H + _GBH_GAP_AFTER_NAME + leader_card_h + _GBH_GAP_AFTER_LEADER + member_card_h) * scale
    return int(max(h, _GB_MIN_H * scale))


def _add_group_block(slide, left, top, width, height, group_name, leader_row, member_rows, photos, scale: float = 1.0, dynamic_map: dict = None, extra_lines: int = 0):
    """繪製工作組別欄位：由上至下堆疊三張獨立卡片——① 組名標頭條 ② 組長獨立卡片 ③ 組員獨立卡片"""
    name_h = int(_GBH_NAME_H * scale)
    y = top

    # ① 組名標頭條（深灰圓角矩形，白字 11pt 粗體）
    name_bar = slide.shapes.add_shape(MSO_SHAPE.RECTANGLE, left, y, width, name_h)
    _set_shape_fill(name_bar, COLOR_HEADER_GRAY)
    name_bar.text_frame.word_wrap = True
    name_bar.text_frame.vertical_anchor = MSO_ANCHOR.MIDDLE
    p = name_bar.text_frame.paragraphs[0]
    p.alignment = PP_ALIGN.CENTER
    run = p.add_run()
    ...
    run.font.color.rgb = RGBColor(0xFF, 0xFF, 0xFF)
    _set_pptx_run_ea_font(run)
    y += name_h + int(_GBH_GAP_AFTER_NAME * scale)

    # ② 組長獨立卡片（own header「組長」+ 白底邊框主體）
    leader_header_h = int(_GBH_LEADER_HEADER_H * scale)
    leader_top_pad = int(_GBH_LEADER_TOP_PAD * scale)
    leader_bottom_pad = int(_GBH_LEADER_BOTTOM_PAD * scale)
    photo_size = int(_GBH_LEADER_PHOTO * scale)
    leader_extra_h = int(_LC_TITLE_LINE_H_UNIT * extra_lines * scale)
    leader_card_h = leader_header_h + leader_top_pad + photo_size + leader_bottom_pad + leader_extra_h

    leader_card = slide.shapes.add_shape(MSO_SHAPE.RECTANGLE, left, y, width, leader_card_h)
    _set_shape_fill(leader_card, COLOR_CARD_BG, line_color=COLOR_CARD_BORDER, line_width=Pt(1))

    leader_header = slide.shapes.add_shape(MSO_SHAPE.RECTANGLE, left, y, width, leader_header_h)
    _set_shape_fill(leader_header, COLOR_GROUP_LEADER_HEADER)
    leader_header.text_frame.word_wrap = True
    leader_header.text_frame.vertical_anchor = MSO_ANCHOR.MIDDLE
    p = leader_header.text_frame.paragraphs[0]
    p.alignment = PP_ALIGN.CENTER
    run = p.add_run()
    run.text = "組長"
    ...
    photo_x = left + int(Inches(0.1) * scale)
    photo_y = y + leader_header_h + leader_top_pad
    if leader_row is not None:
        _add_photo_or_placeholder(slide, photo_x, photo_y, photo_size, get_photo_bytes(photos, leader_row["PhotoName"]))
        info_x = photo_x + photo_size + int(Inches(0.08) * scale)
        info_w = left + width - int(Inches(0.08) * scale) - info_x
        name_line_h = int(photo_size * 0.36)
        title_line_h = int(photo_size * 0.28) + leader_extra_h
        title_text = format_person_title(leader_row, dynamic_map)
        title_font_pt, _ = _estimate_title_lines(title_text, info_w / 914400)
        _add_textbox(slide, info_x, photo_y, info_w, name_line_h,
                     safe_text(leader_row["Name"]), size=10, bold=True, color=COLOR_TEXT_DARK, align=PP_ALIGN.LEFT)
        _add_textbox(slide, info_x, photo_y + name_line_h, info_w, title_line_h,
                     title_text, size=title_font_pt, color=COLOR_TEXT_GRAY, align=PP_ALIGN.LEFT, line_spacing=1.1)
        badges = parse_badges(leader_row["Badges"])
        if badges:
            badge_area_h = photo_size + leader_extra_h - name_line_h - title_line_h  # 卡片內剩餘可用高度，保證晶片不會溢出
            chip_sz = max(1, min(int(Inches(0.2) * scale), badge_area_h))
            _add_badge_row(slide, info_x, photo_y + name_line_h + title_line_h, badges,
                           chip_w=chip_sz, chip_h=chip_sz, gap=int(Inches(0.03) * scale))
    else:
        _add_photo_or_placeholder(slide, photo_x, photo_y, photo_size, None)
        info_x = photo_x + photo_size + int(Inches(0.08) * scale)
        info_w = left + width - int(Inches(0.08) * scale) - info_x
        _add_textbox(slide, info_x, photo_y, info_w, photo_size, "[資料待補]",
                     size=9, color=COLOR_TEXT_GRAY, align=PP_ALIGN.LEFT, anchor=MSO_ANCHOR.MIDDLE)

    y += leader_card_h + int(_GBH_GAP_AFTER_LEADER * scale)

    # ③ 組員獨立卡片（own header「組員」+ 白底邊框主體，逐行列出組員）
    member_header_h = int(_GBH_MEMBER_HEADER_H * scale)
    member_top_pad = int(_GBH_MEMBER_TOP_PAD * scale)
    member_bottom_pad = int(_GBH_MEMBER_BOTTOM_PAD * scale)
    line_h = int(_GBH_MEMBER_LINE_H * scale)
    n_members = max(len(member_rows), 1)
    member_card_h = member_header_h + member_top_pad + line_h * n_members + member_bottom_pad

    member_card = slide.shapes.add_shape(MSO_SHAPE.RECTANGLE, left, y, width, member_card_h)
    _set_shape_fill(member_card, COLOR_CARD_BG, line_color=COLOR_CARD_BORDER, line_width=Pt(1))

    member_header = slide.shapes.add_shape(MSO_SHAPE.RECTANGLE, left, y, width, member_header_h)
    _set_shape_fill(member_header, COLOR_GROUP_MEMBER_HEADER)
    member_header.text_frame.word_wrap = True
    member_header.text_frame.vertical_anchor = MSO_ANCHOR.MIDDLE
    p = member_header.text_frame.paragraphs[0]
    p.alignment = PP_ALIGN.CENTER
    run = p.add_run()
    run.text = "組員"
    run.font.size = Pt(9.5)
    ...
    my = y + member_header_h + member_top_pad
    if member_rows:
        for m in member_rows:
            badges = parse_badges(m["Badges"])
            _add_textbox(slide, left + int(Inches(0.1) * scale), my, width - int(Inches(1.0) * scale), line_h,
                         f"{safe_text(m['Name'])} {format_person_title(m, dynamic_map)}",
                         size=9, color=COLOR_TEXT_DARK, align=PP_ALIGN.LEFT, anchor=MSO_ANCHOR.MIDDLE)
            if badges:
                y_off = int(Inches(0.01) * scale)
                chip_sz = max(1, min(int(Inches(0.2) * scale), line_h - y_off))
                _add_badge_row(slide, left + width - int(Inches(0.9) * scale), my + y_off,
                               badges, chip_w=chip_sz, chip_h=chip_sz, gap=int(Inches(0.03) * scale))
            my += line_h
    else:
        _add_textbox(slide, left + int(Inches(0.1) * scale), my, width - int(Inches(0.2) * scale), line_h,
                     "[資料待補]", size=9, color=COLOR_TEXT_GRAY, align=PP_ALIGN.LEFT)
...
def _stack_height(n_cards: int, scale: float = 1.0, card_gap: Emu = None, extra_lines: int = 0) -> Emu:
    """計算同一欄 n 張卡片垂直堆疊後的總高度（卡片高度依安全網層級縮放，並依 extra_lines 動態加高以容納長職稱文字）"""
    if n_cards <= 0:
        return 0
    gap = card_gap if card_gap is not None else int(Inches(0.14) * scale)
    return _leader_card_height(scale, extra_lines) * n_cards + gap * max(n_cards - 1, 0)


COLOR_CONNECTOR = RGBColor(0x00, 0x00, 0x00)  # 連接線統一改為純黑色（比照 Reference 簡報風格）
...
def _compute_leadership_block_height(zones: dict, scale: float, gap_v: Emu, card_gap: Emu,
                                      n_partners: int = 0, extra_lines: int = 0) -> Emu:
    """計算領導層區塊（左／中軸鏈／右+協力廠商）由頂端到最底部所需總高度，供試算與繪製階段共用同一公式；
    extra_lines 為長職稱/公司文字所需的額外行數，套用至區塊內所有領導卡片，確保尺寸統一。"""
    card_h = _leader_card_height(scale, extra_lines)
    left_h = _stack_height(len(zones["left"]), scale, card_gap, extra_lines)
    right_h = _stack_height(len(zones["right"]), scale, card_gap, extra_lines)
    if n_partners > 0:
        right_h += gap_v + _partner_box_height(n_partners, scale)
    center_own_h = _stack_height(len(zones["center"]), scale, card_gap, extra_lines) if zones["center"] else 0
    chain_h = center_own_h
    if zones["subtop"]:
        chain_h += gap_v + _stack_height(len(zones["subtop"]), scale, card_gap, extra_lines)
    if zones["middle"]:
        chain_h += gap_v + card_h
    return max(left_h, right_h, chain_h)


def _compute_groups_total_height(df: pd.DataFrame, scale: float, group_gap_y: Emu, dynamic_map: dict = None):
    group_names = []
    ...
    margin_x = Inches(0.4)
    gap_x = Inches(0.2)
    avail_w = Inches(13.333) - margin_x * 2 - gap_x * (cols - 1)
    block_w_est = avail_w / cols if cols else avail_w
    row_max_h = {}
    for idx, gname in enumerate(group_names):
        r, _c = divmod(idx, cols)
        n_members = len(df[(df["Layer"] == "GroupMember") & (df["GroupName"] == gname)])
        leader_rows = df[(df["Layer"] == "GroupLeader") & (df["GroupName"] == gname)]
        leader_row = leader_rows.iloc[0] if len(leader_rows) else None
        extra_lines = _group_leader_title_extra_lines(leader_row, block_w_est, scale, dynamic_map)
        bh = _group_block_height(n_members, scale, extra_lines)
...
def _render_column_stack(slide, cx, top_y, rows, card_w, scale, card_gap, photos, conn_width=1.5,
    ...
    for i, r in enumerate(rows):
        _add_leader_card(slide, cx, y, r, photos, card_w=card_w, scale=scale, dynamic_map=dynamic_map, extra_lines=extra_lines)
...
def build_org_chart_pptx(df: pd.DataFrame, stats: dict, photos: dict,
    ...
    # 機關名稱彈性解析：UI 側邊欄優先，留空則自動讀取 Excel Layer=='Owner' 資料
    host_org, client_org = resolve_org_banner_info(df, host_org, client_org)

    zones = _leadership_zones(df, swap_lr=swap_lr)
    used_badges = get_used_badges(df)  # 圖例：本案實際出現的徽章全數列出（含碩/博），依全域排序規範排列
    dynamic_map = build_dynamic_badge_map(df)  # 若 Excel 提供 Licenses 欄位，動態對照優先於靜態 BADGE_MAP
    legend_h = compute_legend_height(len(used_badges))
    summary = build_summary_stats(df, team_expertise_override)
    fixed_top_row_h = compute_fixed_top_row_height(legend_h)  # 圖例/Summary 卡片列本身高度，不隨安全網縮放
    partner_rows = get_subcontractor_rows(df)
    n_partners = len(partner_rows)

    # 動態估算領導層卡片因長職稱/公司名稱需要的額外換行行數（以較窄的 2.6in 保守估計寬度試算，
    # 確保無論最終 card_w 落在 2.6in 或 3.0in，長文字皆能 100% 完整呈現不溢出）。
    # 統一取全體領導層人員（左/中/SubTop/右/計畫經理）中的「最大需求」，套用至整層卡片，維持尺寸一致。
    _all_leadership_rows = (zones["left"] + zones["center"] + zones["subtop"]
                             + zones["right"] + zones["middle"])
    extra_lines = 0
    for _r in _all_leadership_rows:
        extra_lines = max(extra_lines, _leader_card_extra_title_lines(_r, Inches(2.6), dynamic_map))
    ...
    def total_height_for(scale, gap_v, card_gap, group_gap_y):
        banner_part = compute_banner_unit_height(host_org, client_org, scale, extra_lines)
        top_row_final_gap = int(_TOP_ROW_FINAL_GAP_BASE * scale)
        leadership_h = _compute_leadership_block_height(zones, scale, gap_v, card_gap, n_partners, extra_lines)
        groups_h, _names, _cols = _compute_groups_total_height(df, scale, group_gap_y, dynamic_map)
        return (fixed_top_row_h + banner_part + top_row_final_gap
                + leadership_h + gap_v + groups_h + FOOTER_H + BOTTOM_MARGIN)

    # ---------- 三層安全網：① 標準間距 → ② 最小間距 → ③ 等比例幾何縮放（字級固定不變） ----------
    # 機關卡片（banner）與其相關間距皆已納入可壓縮預算，與領導卡片、分組卡片同步等比例縮放，
    # 避免機關卡片尺寸統一化後佔用過多固定預留空間，確保極端資料量下仍 100% 不溢出。
    ...
    if total_nom <= available:
        ...
        if total_min <= available:
            ...
            fixed_part = fixed_top_row_h + FOOTER_H + BOTTOM_MARGIN
    ...
    # 依最終 scale 重新計算機關列與領導層起始位置（machine of record，供繪製階段使用）
    banner_h_final = compute_banner_unit_height(host_org, client_org, scale, extra_lines)
    top_section_h = banner_h_final + fixed_top_row_h + int(_TOP_ROW_FINAL_GAP_BASE * scale)
    ...
    # --- 中軸領導層卡片尺寸：先行計算，供機關卡片與領導卡片統一套用同一尺寸 ---
    ...
    # --- 頂部黃金三元素：機關列 / 圖例 / Summary 卡片（機關卡片寬高與領導卡片完全統一，且隨 scale 同步縮放） ---
    _add_compact_org_banner(slide, slide_w, host_org, client_org, card_w, card_h)
    banner_h = card_h if (host_org or client_org) else Inches(0)
    banner_gap = int(_BANNER_GAP_BASE * scale) if banner_h else Inches(0)
    top_row_y = banner_h + banner_gap

    leadership_row_y = top_section_h

    # 主管機關 -> 計畫主持人：僅在有填寫主辦/委辦機關時繪製垂直連接線（彈性渲染，未填寫則自動省略不留空白）。
    # 刻意在圖例／Summary 卡片「之前」繪製此連接線，讓後續繪製的不透明卡片視覺上自然覆蓋線段經過的區域，
    # 確保連接線絕不會貫穿 Summary 卡片（即使兩者在水平座標上有重疊）。
    if (host_org or client_org) and zones["center"]:
        try:
            _add_connector(slide, center_cx, banner_h, center_cx, leadership_row_y, straight=True, width_pt=conn_width)
        except Exception:
            pass

    _add_legend(slide, Inches(0.4), top_row_y, used_badges, dynamic_map)

    summary_w = Inches(7.2)
    ...
    left_first_bottom, left_last_bottom = _render_column_stack(
        slide, left_cx, leadership_row_y, zones["left"], card_w, scale, card_gap, photos, conn_width, dynamic_map, extra_lines)
    center_first_bottom, center_last_bottom = _render_column_stack(
        slide, center_cx, leadership_row_y, zones["center"], card_w, scale, card_gap, photos, conn_width, dynamic_map, extra_lines)

    right_first_bottom, right_last_bottom = _render_column_stack(
        slide, right_cx, leadership_row_y, zones["right"], card_w, scale, card_gap, photos, conn_width, dynamic_map, extra_lines)
    ...
    if zones["subtop"]:
        ...
        _st_first, st_last_bottom = _render_column_stack(
            slide, center_cx, subtop_top, zones["subtop"], card_w, scale, card_gap, photos, conn_width, dynamic_map, extra_lines)
    ...
    if zones["middle"]:
        ...
        try:
            ...
            pass
        _add_leader_card(slide, center_cx, middle_top, zones["middle"][0], photos, card_w=card_w, scale=scale, dynamic_map=dynamic_map, extra_lines=extra_lines)
        middle_bottom = middle_top + card_h

    leadership_block_h = _compute_leadership_block_height(zones, scale, gap_v, card_gap, n_partners, extra_lines)
    groups_top = leadership_row_y + leadership_block_h + gap_v

    # --- 工作組別區 ---
    groups_h, group_names, cols = _compute_groups_total_height(df, scale, group_gap_y, dynamic_map)
    margin_x = Inches(0.4)
    gap_x = Inches(0.2)
    ...
    for idx, gname in enumerate(group_names):
        ...
        group_extra_lines = _group_leader_title_extra_lines(leader_row, block_w, scale, dynamic_map)
        block_h = _group_block_height(len(member_rows), scale, group_extra_lines)
        group_infos.append({"r": r, "c": c, "gname": gname, "leader_row": leader_row,
                             "member_rows": member_rows, "block_h": block_h, "extra_lines": group_extra_lines})
    ...
    for info in group_infos:
        ...
        _add_group_block(slide, bx, by, block_w, info["block_h"], info["gname"],
                          ...
                          dynamic_map=dynamic_map, extra_lines=info["extra_lines"])
    ...
    # --- 頁尾統計時間軸（底色/數字色/文字色完全比照 Reference 簡報；大數字為刻意保留的視覺焦點，不受 9-12pt 文字規範限制） ---
    footer = slide.shapes.add_shape(MSO_SHAPE.RECTANGLE, 0, footer_top, slide_w, FOOTER_H)
    _set_shape_fill(footer, COLOR_FOOTER_BG)
    stat_items = [stats.get("avg_years", ""), stats.get("masters", ""), stats.get("doctors", ""),
    ...
    for i, item in enumerate(stat_items):
        ...
        _add_textbox(slide, cx0, footer_top + Inches(0.05), seg_w, Inches(0.4), number,
                     size=20, bold=True, color=COLOR_STAT_NUMBER)
        _add_textbox(slide, cx0, footer_top + Inches(0.42), seg_w, Inches(0.28), label,
                     size=10, color=RGBColor(0x00, 0x00, 0x00))
...
def build_bio_docx(df: pd.DataFrame, photos: dict) -> io.BytesIO:
    ...
    for _, r in df2.iterrows():
        ...
        cert_names = [badge_full_name(b, dynamic_map) for b in badges]
...
if "stats" not in st.session_state:
    st.session_state.stats = {"avg_years": "0 平均年資", "masters": "0 碩士", "doctors": "0 博士",
...
with st.expander("⚙️ 標案參數設定面板", expanded=False):
    ...
    with col_org:
        st.markdown("**機關名稱**")
        st.session_state.host_org = st.text_input(
            "主辦機關", value=st.session_state.host_org,
            help="留空時會自動讀取 Excel 中 Layer=='Owner' 的資料（Role 含「委辦」者視為委辦機關，其餘視為主辦機關）。")
        st.session_state.client_org = st.text_input(
            "委辦機關", value=st.session_state.client_org,
            help="留空時會自動讀取 Excel 中 Layer=='Owner' 的資料，若僅需覆蓋其中一項，另一項仍會沿用 Excel。")
    ...
    with col_stats:
        ...
        with s2:
            st.session_state.stats["masters"] = st.text_input("碩士人數", st.session_state.stats["masters"])
            st.session_state.stats["groups"] = st.text_input("專業分組數", st.session_state.stats["groups"])
        with s3:
            st.session_state.stats["doctors"] = st.text_input("博士人數", st.session_state.stats["doctors"])
...
with tab1:
    ...
    dynamic_map_preview = build_dynamic_badge_map(df)
    ...
    def person_card(row, small=False):
        ...
        return (
            ...
            f"<div style='font-size:11px;color:#666;'>{format_person_title(row, dynamic_map_preview)}</div>"
            f"{company_html}"
    ...
    resolved_host_org, resolved_client_org = resolve_org_banner_info(df, host_org, client_org)

    org_header = ""
    if resolved_host_org or resolved_client_org:
        org_items = []
        if resolved_host_org:
            org_items.append(("主辦機關", resolved_host_org))
        if resolved_client_org:
            org_items.append(("委辦機關", resolved_client_org))
        cards_html = "".join(
            "<div style='flex:1;max-width:320px;border:1px solid #000;border-radius:0;overflow:hidden;'>"
            "<div style='background:#00A3E0;color:#fff;text-align:center;font-weight:700;"
            f"font-size:13px;padding:6px;'>{label}</div>"
            "<div style='background:#fff;color:#222;text-align:center;font-weight:700;"
            f"font-size:14px;padding:10px;'>{name}</div></div>"
            for label, name in org_items
        )
        org_header = (
    ...
    for zone_key, col, people in [("left", zone_l, zones["left"]),
        ...
        with col:
            ...
            for person in people:
                st.markdown(f"<div style='text-align:center;background:#F26522;color:#fff;border-radius:6px;"
            ...
            if zone_key == "right" and partner_rows_preview:
                ...
                st.markdown(
                    ...
                    "<div style='background:#1F3A5F;color:#fff;text-align:center;font-weight:700;"
    ...
    if middles:
        ...
        with mc2:
            st.markdown(f"<div style='text-align:center;background:#4A4A4A;color:#fff;border-radius:6px;"
            ...
            if len(members):
                ...
                for _, m in members.iterrows():
                    ...
                    st.markdown(f"<div style='display:flex;justify-content:space-between;background:#fff;"
                                ...
                                f"<span>{safe_text(m['Name'])} {format_person_title(m, dynamic_map_preview)}</span>"
                                f"<span>{badge_html(badges)}</span></div>", unsafe_allow_html=True)

    st.markdown("---")
    stats = st.session_state.stats
    fcols = st.columns(5)
    for i, key in enumerate(["avg_years", "masters", "doctors", "technicians", "groups"]):
        val = stats.get(key, "")
        parts = val.split(" ", 1)
        num = parts[0] if parts else ""
        lbl = parts[1] if len(parts) > 1 else ""
        with fcols[i]:
            st.markdown(f"<div style='text-align:center;background:#5FA4BE;border-radius:0;padding:10px;'>"
                        f"<div style='font-size:24px;font-weight:800;color:#984807;'>{num}</div>"
                        f"<div style='font-size:12px;color:#000;'>{lbl}</div></div>", unsafe_allow_html=True)
