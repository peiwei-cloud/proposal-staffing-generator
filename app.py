...
def compute_footer_stats(df: pd.DataFrame) -> dict:
    """頁尾統計：碩士／博士人數統計僅依 Badges 欄位（分別計算「碩」「博」代碼），
    不再混用 Degree 文字擷取，確保與組織圖卡片上實際呈現的徽章晶片數量完全一致。"""
    avg_years = 0
    masters = 0
    doctors = 0
    technicians = 0
    ...
    try:
        ...
        avg_years = 0
    try:
        masters = int(df["Badges"].apply(lambda b: "碩" in parse_badges(b)).sum())
    except Exception:
        masters = 0
    try:
        doctors = int(df["Badges"].apply(lambda b: "博" in parse_badges(b)).sum())
    except Exception:
        doctors = 0
    try:
