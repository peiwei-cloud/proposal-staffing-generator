...
st.set_page_config(page_title="標案專案人力與組織圖產生器", layout="wide")
...
st.title("📋 標案專案人力與組織圖 Web 產生器")
st.caption("上傳人員資料 Excel 與照片 Zip，即可預覽並一鍵匯出組織圖、人力配置表、人員經歷說明書")

# ---------------- Sidebar ----------------
with st.sidebar:
    st.header("① 資料上傳")
    excel_file = st.file_uploader("上傳 Template_Staffing.xlsx", type=["xlsx"])
    if excel_file is not None:
        try:
            st.session_state.df = load_staffing_excel(excel_file)
            st.success(f"已讀取 {len(st.session_state.df)} 筆人員資料")
        except Exception as e:
            st.error(f"Excel 讀取失敗：{e}")

    zip_file = st.file_uploader("上傳 Photos.zip（大頭照）", type=["zip"])
    if zip_file is not None:
        st.session_state.photos = load_photos_zip(zip_file)
        st.success(f"已讀取 {len(st.session_state.photos)} 張照片")

    st.divider()
    st.header("② 頂部機關資訊（選填）")
    host_org = st.text_input("主辦機關", value="")
    client_org = st.text_input("委辦機關", value="")
    ...
    if df is None:
        st.info("請先上傳 Excel 資料以啟用匯出功能")
    else:
        try:
            pptx1 = build_org_chart_pptx(df, st.session_state.stats, st.session_state.photos,
                                          host_org, client_org)
            st.download_button("⬇️ 匯出 Output 1：組織架構圖 (.pptx)", data=pptx1,
        ...
        try:
            ...
            st.download_button("⬇️ 匯出 Output 2：人力配置表 (.docx)", data=docx2,
                                file_name="主要工作人力配置表.docx",
                                mime="application/vnd.openxmlformats-officedocument.wordprocessingml.document",
                                use_container_width=True)
        except Exception as e:
            st.error(f"Output 2 (docx) 產生失敗：{e}")

        try:
            pptx2 = build_staffing_table_pptx(df)
            st.download_button("⬇️ 匯出 Output 2：人力配置表 (.pptx)", data=pptx2,
        ...
        try:
            ...
            st.download_button("⬇️ 匯出 Output 3：人員經歷說明書 (.docx)", data=docx3,
...
if df is None:
    st.warning("👈 請先於左側上傳人員資料 Excel（Template_Staffing.xlsx 格式）")
    st.stop()
...
# ---- Tab 1: 組織圖 HTML 概覽預覽 ----
with tab1:
    st.caption("以下為結構化概覽預覽，實際版面、字型與間距請以左側下載之 .pptx 檔案為準。")
