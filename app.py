...
def get_used_badges(df: pd.DataFrame) -> list:
    """掃描整份 Excel（所有人員，不分 Layer）實際出現過的徽章代碼，依首次出現順序回傳。
    圖例僅根據此清單繪製，避免顯示本案未使用的預設徽章。"""
    seen_set = set()
    seen = []
    for b in df["Badges"]:
...
def _compute_leader_top(host_org: str, client_org: str, legend_h: Emu) -> Emu:
    """統一計算領導層起始 y 座標：取「頂部機關列所需高度」與「左上角圖例所需高度」兩者中較高者，
    此函式在試算階段與實際繪製階段皆呼叫，確保兩階段結果一致，避免重疊。"""
    banner_based = Inches(1.15) if (host_org or client_org) else Inches(0.55)
    legend_based = _LEGEND_TOP_PAD + legend_h + Inches(0.15) if legend_h else Inches(0)
    return max(banner_based, legend_based)
...
def _add_connector(slide, x1, y1, x2, y2, straight=True):
...
def build_org_chart_pptx(df: pd.DataFrame, stats: dict, photos: dict,
    ...
    used_badges = get_used_badges(df)  # 圖例僅列出本案實際出現的徽章
    legend_h = compute_legend_height(len(used_badges))

    # ---------- 第一階段：純數值試算版面高度，避免區塊互相重疊 ----------
    leader_top_calc = _compute_leader_top(host_org, client_org, legend_h)
    ...
    # --- 左上角圖例（僅列出本案實際出現的徽章） ---
    _add_legend(slide, Inches(0.4), Inches(0.12), used_badges)
    ...
    leader_top = leader_top_calc  # 與試算階段共用同一函式結果，確保一致不重疊
    ...
    def _render_stack(zone_key, rows):
        """垂直堆疊繪製同一欄的人員卡片，同一欄內相鄰卡片之間以直線連接線相連（如 Top -> SubTop）。
        回傳 (該欄中心x, 最後一張卡片的底部y)，供後續連接至計畫經理卡片使用。"""
        cx = zone_cx[zone_key]
        y = leader_top
        prev_bottom = None
        for r in rows:
            ...
            if prev_bottom is not None:
                try:
                    _add_connector(slide, cx, prev_bottom, cx, y, straight=True)
                except Exception:
                    pass
            prev_bottom = y + h
            y += h + CARD_STACK_GAP
        return cx, prev_bottom

    left_cx, left_last_bottom = _render_stack("left", zones["left"])
    center_cx, center_last_bottom = _render_stack("center", center_stack)
    right_cx, right_last_bottom = _render_stack("right", zones["right"])
    ...
    if zones["middle"]:
        ...
        middle_bottom = middle_top + _leader_card_height(zones["middle"][0])
    groups_top = groups_top_calc

    # 連接線：左／中／右三欄「最後一張卡片」下緣 -> 計畫經理卡片上緣
    # 中欄與計畫經理同一垂直線（SubTop -> Middle）用直線；左／右欄因 x 不同，改用直角(elbow)連接線
    if zones["middle"]:
        for cx, last_bottom in [(left_cx, left_last_bottom), (center_cx, center_last_bottom),
                                 (right_cx, right_last_bottom)]:
            if last_bottom is None:
                continue
            try:
                is_same_column = abs(cx - middle_cx) < Inches(0.05)
                _add_connector(slide, cx, last_bottom, middle_cx, middle_top, straight=is_same_column)
            except Exception:
                pass
    ...
    # 連接線：計畫經理 -> 各組別（僅連接第一列，第二列以下視覺上已隸屬於整體分組區塊）
    # 採用主幹（Middle 下緣 -> 匯流排）+ 水平匯流排 + 垂直分支（匯流排 -> 各組別上緣）的樹狀直角連接
    if zones["middle"] and middle_bottom is not None:
        ...
        if row0_centers:
            bus_y = groups_top - Inches(0.13)
            try:
                _add_connector(slide, middle_cx, middle_bottom, middle_cx, bus_y, straight=True)
                if len(row0_centers) > 1:
                    _add_connector(slide, min(row0_centers), bus_y, max(row0_centers), bus_y, straight=True)
                for gc in row0_centers:
                    _add_connector(slide, gc, bus_y, gc, groups_top, straight=True)
            except Exception:
                pass
