...
def compute_footer_stats(df: pd.DataFrame) -> dict:
    ...
    try:
        ...
        avg_years = 0
    try:
        degree_match = df["Degree"].str.contains("碩士|博士", na=False)
        badge_match = df["Badges"].apply(lambda b: bool(set(parse_badges(b)) & {"碩", "博"}))
        masters = int((degree_match | badge_match).sum())
    except Exception:
        masters = 0
    try:
...
_ORG_BANNER_HEADER_H = Inches(0.24)
_ORG_BANNER_BODY_H = Inches(0.38)
_ORG_BANNER_H = _ORG_BANNER_HEADER_H + _ORG_BANNER_BODY_H
...
def compute_top_section_height(host_org: str, client_org: str, legend_h: Emu) -> Emu:
    ...
    banner_gap = Inches(0.08) if banner_h else Inches(0)
...
def _add_compact_org_banner(slide, slide_w, host_org: str, client_org: str):
    """主辦／委辦機關卡片：水藍色標頭條（顯示「主辦機關」/「委辦機關」）+ 白底卡片顯示機關名稱，比照領導卡片視覺風格"""
    ...
    box_w = Inches(3.6)
    gap = Inches(0.3)
    total_w = box_w * len(items) + gap * (len(items) - 1)
    start_x = int((slide_w - total_w) / 2)
    header_h = _ORG_BANNER_HEADER_H
    body_h = _ORG_BANNER_BODY_H

    for i, (label, val) in enumerate(items):
        bx = start_x + i * (box_w + gap)
        # 白底卡片主體
        card = slide.shapes.add_shape(MSO_SHAPE.RECTANGLE, bx, 0, box_w, header_h + body_h)
        _set_shape_fill(card, COLOR_CARD_BG, line_color=COLOR_CARD_BORDER, line_width=Pt(1))
        # 水藍色標頭
        header = slide.shapes.add_shape(MSO_SHAPE.RECTANGLE, bx, 0, box_w, header_h)
        ...
        _set_pptx_run_ea_font(run)
        # 機關名稱（黑字 12pt 粗體，置中）
        _add_textbox(slide, bx, header_h, box_w, body_h, val, size=12, bold=True,
...
def _add_group_block(slide, left, top, width, height, group_name, leader_row, member_rows, photos, scale: float = 1.0):
    ...
    if leader_row is not None:
        ...
        if badges:
            _add_badge_row(slide, info_x, photo_y + name_line_h + title_line_h, badges,
                           chip_w=Inches(0.2), chip_h=Inches(0.2), gap=Inches(0.03))
    else:
    ...
    if member_rows:
        for m in member_rows:
            ...
            if badges:
                _add_badge_row(slide, left + width - int(Inches(0.9) * scale), my + int(Inches(0.01) * scale),
                               badges, chip_w=Inches(0.2), chip_h=Inches(0.2), gap=Inches(0.03))
            my += line_h
    else:
