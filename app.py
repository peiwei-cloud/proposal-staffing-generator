...
def compute_footer_stats(df: pd.DataFrame) -> dict:
    ...
    try:
        ...
        doctors = 0
    try:
        technicians = int(df["Badges"].apply(lambda b: "技" in parse_badges(b)).sum())
    except Exception:
        technicians = 0
    try:
...
def compute_banner_unit_height(host_org: str, client_org: str, scale: float = 1.0) -> Emu:
    """機關卡片（含其下方間距）所需高度，隨 scale 等比例縮放，與領導卡片同步壓縮，
    確保三層安全網觸發時機關卡片與領導卡片仍維持相同尺寸，且不會因固定預留過大而導致下方內容溢出。"""
    if not (host_org or client_org):
        return Inches(0)
    return int(_leader_card_height(scale) + _BANNER_GAP_BASE * scale)
...
SUMMARY_PE_BADGES = {"水", "土", "環", "保", "結", "景", "技"}
SUMMARY_PMP_BADGES = {"P", "品", "安", "採"}
# 技師「科別」徽章（不含通用「技師執照」代碼「技」），用於 Summary 分項統計；
...
def get_technician_suffix(row, dynamic_map: dict = None) -> str:
    """判斷該人員是否具備技師身份，回傳應附加於職稱後的技師稱謂全名（如「水利技師」）；
    無技師身份則回傳空字串，不可硬加無謂稱謂。
    判斷優先序：① Badges 含具體科別代碼（水/土/環…）→ 該科別全名；
    ② Badges 含通用「技」代碼 → 「技師執照」原名沿用（不強加特定科別）；
    ③ Title／Expertise 文字提及任一科別全名 → 該科別全名（CV 文字補強）。"""
    badges = set(parse_badges(row.get("Badges", "")))
    for code in sort_badges(list(badges & SUMMARY_PE_SPECIALTY_BADGES)):
        return badge_full_name(code, dynamic_map)
    if "技" in badges:
        return "技師"
    text_blob = f"{row.get('Title','') or ''}{row.get('Expertise','') or ''}"
    for code in sort_badges(list(SUMMARY_PE_SPECIALTY_BADGES)):
...
def _add_leader_card(slide, cx, top, row, photos, card_w=Inches(2.8), scale: float = 1.0, dynamic_map: dict = None):
    ...
    title_text = clean_separator_join([title_with_suffix, company], sep="｜")
...
def _group_block_height(n_members: int, scale: float = 1.0) -> Emu:
    ...
    同一組具名常數，避免試算與繪製結果不一致。"""
    leader_card_h = _GBH_LEADER_HEADER_H + _GBH_LEADER_TOP_PAD + _GBH_LEADER_PHOTO + _GBH_LEADER_BOTTOM_PAD
...
def _add_group_block(slide, left, top, width, height, group_name, leader_row, member_rows, photos, scale: float = 1.0, dynamic_map: dict = None):
    ...
    leader_card_h = leader_header_h + leader_top_pad + photo_size + leader_bottom_pad
    ...
    if leader_row is not None:
        ...
        title_line_h = int(photo_size * 0.28)
        ...
        _add_textbox(slide, info_x, photo_y + name_line_h, info_w, title_line_h,
                     format_person_title(leader_row, dynamic_map), size=9, color=COLOR_TEXT_GRAY, align=PP_ALIGN.LEFT)
        badges = parse_badges(leader_row["Badges"])
        if badges:
            badge_area_h = photo_size - name_line_h - title_line_h  # 卡片內剩餘可用高度，保證晶片不會溢出
...
def _render_column_stack(slide, cx, top_y, rows, card_w, scale, card_gap, photos, conn_width=1.5, dynamic_map: dict = None):
    ...
    for i, r in enumerate(rows):
        _add_leader_card(slide, cx, y, r, photos, card_w=card_w, scale=scale, dynamic_map=dynamic_map)
...
def build_org_chart_pptx(df: pd.DataFrame, stats: dict, photos: dict,
    ...
    def total_height_for(scale, gap_v, card_gap, group_gap_y):
        banner_part = compute_banner_unit_height(host_org, client_org, scale)
    ...
    banner_h_final = compute_banner_unit_height(host_org, client_org, scale)
    ...
    left_first_bottom, left_last_bottom = _render_column_stack(
        slide, left_cx, leadership_row_y, zones["left"], card_w, scale, card_gap, photos, conn_width, dynamic_map)
    center_first_bottom, center_last_bottom = _render_column_stack(
        slide, center_cx, leadership_row_y, zones["center"], card_w, scale, card_gap, photos, conn_width, dynamic_map)

    right_first_bottom, right_last_bottom = _render_column_stack(
        slide, right_cx, leadership_row_y, zones["right"], card_w, scale, card_gap, photos, conn_width, dynamic_map)
    ...
    if zones["subtop"]:
        ...
        _st_first, st_last_bottom = _render_column_stack(
            slide, center_cx, subtop_top, zones["subtop"], card_w, scale, card_gap, photos, conn_width, dynamic_map)
    ...
    if zones["middle"]:
        ...
        try:
            ...
            pass
        _add_leader_card(slide, center_cx, middle_top, zones["middle"][0], photos, card_w=card_w, scale=scale, dynamic_map=dynamic_map)
    ...
    for info in group_infos:
        ...
        _add_group_block(slide, bx, by, block_w, info["block_h"], info["gname"],
                          info["leader_row"], info["member_rows"], photos, scale=scale, dynamic_map=dynamic_map)
