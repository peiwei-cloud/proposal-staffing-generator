...
_PB_MIN_H = Inches(0.5)
_PB_LABEL_W = Inches(0.32)


def _partner_box_height(n_partners: int, scale: float = 1.0) -> Emu:
    if n_partners <= 0:
        return Inches(0)
    h = (_PB_TOP_PAD + _PB_LINE_H * n_partners + _PB_BOTTOM_PAD) * scale
    return int(max(h, _PB_MIN_H * scale))


def _add_partner_box(slide, x, y, w, partner_rows, scale: float = 1.0):
    """繪製協力廠商／協力團隊虛線外框區塊：左側直式標籤「協/力/廠/商」，右側逐行「項目：廠商名稱」。
    寬度／高度依實際筆數動態調整（滾動式適配版面密度），不佔用固定空間。"""
    n = len(partner_rows)
    if n == 0:
        return
    ...
    box.fill.solid()
    box.fill.fore_color.rgb = RGBColor(0xEA, 0xF4, 0xFD)
    box.line.color.rgb = COLOR_NAVY_LIGHT
    box.line.width = Pt(1.1)
    box.line.dash_style = MSO_LINE_DASH_STYLE.DASH
    box.shadow.inherit = False

    label_w = int(_PB_LABEL_W * scale)
    label_text = "\n".join(list("協力廠商"))
    _add_textbox(slide, x, y, label_w, h, label_text, size=10, bold=True,
                 color=COLOR_NAVY, align=PP_ALIGN.CENTER, line_spacing=1.0)

    content_x = x + label_w + int(Inches(0.06) * scale)
    content_w = w - label_w - int(Inches(0.14) * scale)
    line_h = int(_PB_LINE_H * scale)
    cy = y + int(_PB_TOP_PAD * scale)
    for r in partner_rows:
        ...
        _add_textbox(slide, content_x, cy, content_w, line_h, line,
                     size=9.5, bold=False, color=COLOR_TEXT_DARK, align=PP_ALIGN.LEFT, anchor=MSO_ANCHOR.MIDDLE)
        cy += line_h
...
def build_org_chart_pptx(df: pd.DataFrame, stats: dict, photos: dict,
    ...
    # --- 協力廠商／協力團隊：固定於右欄卡片下方，寬度延伸至右側邊界，長寬依實際筆數動態調整 ---
    if partner_rows:
        partner_top = (right_last_bottom if right_last_bottom is not None else leadership_row_y) + gap_v
        partner_left = right_cx - card_w / 2
        partner_w = slide_w - Inches(0.4) - partner_left
        _add_partner_box(slide, partner_left, partner_top, partner_w, partner_rows, scale)
        ...
        with col:
            ...
            if zone_key == "right" and partner_rows_preview:
                ...
                for r in partner_rows_preview:
                    ...
                    company = safe_text(r.get("Name", ""), "[廠商待補]")
                    line = f"{item}：{company}" if item else company
                    lines.append(f"<div style='font-size:10.5px;color:#333;padding:1px 0;'>{line}</div>")
                st.markdown(
                    "<div style='border:1.5px dashed #3E5C86;background:#EAF4FD;border-radius:8px;"
                    "padding:6px 8px;margin-top:6px;'>"
                    "<div style='font-size:10px;font-weight:700;color:#1F3A5F;margin-bottom:3px;'>協力廠商</div>"
