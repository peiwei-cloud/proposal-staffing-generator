...
# 領導卡片版面常數（橫向版型：照片在左、資訊在右；scale 供第三層安全網等比例壓縮使用）
...
def _compute_leadership_block_height(zones: dict, scale: float, gap_v: Emu, card_gap: Emu) -> Emu:
    """計算領導層區塊（左／中軸鏈／右）由頂端到最底部所需總高度，供試算與繪製階段共用同一公式"""
...
def _render_column_stack(slide, cx, top_y, rows, card_w, scale, card_gap, photos):
    ...
    for i, r in enumerate(rows):
        ...
        if last_bottom is not None:
            try:
                _add_connector(slide, cx, last_bottom, cx, y, straight=True)
            except Exception:
                pass
...
def build_org_chart_pptx(df: pd.DataFrame, stats: dict, photos: dict,
                          ...
                          team_expertise_override: str = "") -> io.BytesIO:
    ...
    def total_height_for(scale, gap_v, card_gap, group_gap_y):
        leadership_h = _compute_leadership_block_height(zones, scale, gap_v, card_gap)
    ...
    left_first_bottom, left_last_bottom = _render_column_stack(
        slide, left_cx, leadership_row_y, zones["left"], card_w, scale, card_gap, photos)
    center_first_bottom, center_last_bottom = _render_column_stack(
        slide, center_cx, leadership_row_y, zones["center"], card_w, scale, card_gap, photos)
    right_first_bottom, right_last_bottom = _render_column_stack(
        slide, right_cx, leadership_row_y, zones["right"], card_w, scale, card_gap, photos)
    ...
    if zones["center"]:
        ...
        try:
            if zones["left"]:
                ...
                _add_connector(slide, left_right_edge, mid_y, center_left_edge, mid_y, straight=True)
            if zones["right"]:
                right_left_edge = right_cx - card_w // 2
                _add_connector(slide, center_right_edge, mid_y, right_left_edge, mid_y, straight=True)
        except Exception:
            pass
    ...
    if zones["subtop"]:
        ...
        try:
            _add_connector(slide, center_cx, y_cursor, center_cx, subtop_top, straight=True)
        except Exception:
            pass
        _st_first, st_last_bottom = _render_column_stack(
            slide, center_cx, subtop_top, zones["subtop"], card_w, scale, card_gap, photos)
    ...
    if zones["middle"]:
        ...
        try:
            _add_connector(slide, center_cx, y_cursor, center_cx, middle_top, straight=True)
        except Exception:
            pass
    ...
    leadership_block_h = _compute_leadership_block_height(zones, scale, gap_v, card_gap)
    ...
    if zones["middle"] and middle_bottom is not None:
        ...
        if row0_centers:
            ...
            try:
                _add_connector(slide, center_cx, middle_bottom, center_cx, bus_y, straight=True)
