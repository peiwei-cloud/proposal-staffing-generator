...
def get_effective_badges(row) -> set:
    ...
    for code in SUMMARY_PE_SPECIALTY_BADGES:
        full_name = BADGE_MAP.get(code, "")
        if full_name and full_name in text_blob:
            inferred.add(code)
...
def build_summary_stats(df: pd.DataFrame, expertise_override: str = "") -> dict:
    ...
    effective_badges_list = [get_effective_badges(r) for _, r in df.iterrows()]
    ...
    breakdown_parts = [
        f"{BADGE_MAP.get(code, code)} {specialty_counts[code]} 人"
...
def _add_leader_card(slide, cx, top, row, photos, card_w=Inches(2.8), scale: float = 1.0):
    """橫向領導卡片：頂部職稱標頭列（依 Layer 分級配色），下方左側照片、右側姓名/職稱與公司/徽章由上至下排列"""
    header_h = int(_LC_HEADER_H * scale)
...
def _add_group_block(slide, left, top, width, height, group_name, leader_row, member_rows, photos, scale: float = 1.0):
    ...
    if leader_row is not None:
        ...
        _add_textbox(slide, info_x, photo_y + name_line_h, info_w, title_line_h,
                     safe_text(leader_row["Title"], ""), size=9, color=COLOR_TEXT_GRAY, align=PP_ALIGN.LEFT)
