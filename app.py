...
def _add_compact_org_banner(slide, slide_w, host_org: str, client_org: str):
    ...
    for i, (label, val) in enumerate(items):
        ...
        # 白底卡片主體
        card = slide.shapes.add_shape(MSO_SHAPE.ROUNDED_RECTANGLE, bx, 0, box_w, header_h + body_h)
        card.adjustments[0] = 0.06
        _set_shape_fill(card, COLOR_CARD_BG, line_color=COLOR_CARD_BORDER, line_width=Pt(1))
        # 水藍色標頭
        header = slide.shapes.add_shape(MSO_SHAPE.ROUNDED_RECTANGLE, bx, 0, box_w, header_h)
...
def build_summary_stats(df: pd.DataFrame, expertise_override: str = "") -> dict:
    ...
    - total：總團隊人數
    - pe_experts：具備【水/土/環/保/結/景/技】任一徽章之總人數（技師級專家）
    - pmp_managers：具備【P/品/安/採】任一徽章之總人數（PMP/品管/安衛專業人才）
    - expertise：專業涵蓋領域文字（自動萃取或手動覆寫）
    """
    total = len(df)

    def _has_any(badge_str, target_set):
        codes = set(parse_badges(badge_str))
        return bool(codes & target_set)

    try:
        pe_experts = int(df["Badges"].apply(lambda b: _has_any(b, SUMMARY_PE_BADGES)).sum())
    except Exception:
        pe_experts = 0
    try:
        pmp_managers = int(df["Badges"].apply(lambda b: _has_any(b, SUMMARY_PMP_BADGES)).sum())
    except Exception:
        pmp_managers = 0
    ...
    return {"total": total, "pe_experts": pe_experts, "pmp_managers": pmp_managers, "expertise": expertise_str}
...
def _add_summary_card(slide, x, y, w, summary: dict):
    ...
    line1 = f"配置 {summary['pe_experts']} 位技師級專家及 {summary['pmp_managers']} 位 PMP/品管/安衛專業人才，具備專業執照與高階碩博士學歷"
    line2 = f"專業涵蓋：{summary['expertise']}"
    _add_textbox(slide, text_x, y + Inches(0.1), text_w, Inches(0.42), line1,
                 size=9.5, bold=False, color=COLOR_TEXT_DARK, align=PP_ALIGN.LEFT, wrap=True)
    _add_textbox(slide, text_x, y + Inches(0.56), text_w, Inches(0.34), line2,
...
def _add_leader_card(slide, cx, top, row, photos, card_w=Inches(2.8), scale: float = 1.0):
    ...
    # 職稱標頭：Top/Advisor/SubTop 橙色、Middle 深灰（僅上緣圓角視覺，矩形疊加圓角卡片頂部即可達成一致外觀）
...
def _add_group_block(slide, left, top, width, height, group_name, leader_row, member_rows, photos, scale: float = 1.0):
    ...
    name_bar = slide.shapes.add_shape(MSO_SHAPE.ROUNDED_RECTANGLE, left, y, width, name_h)
    name_bar.adjustments[0] = 0.5
    ...
    leader_card = slide.shapes.add_shape(MSO_SHAPE.ROUNDED_RECTANGLE, left, y, width, leader_card_h)
    leader_card.adjustments[0] = 0.08
    _set_shape_fill(leader_card, COLOR_CARD_BG, line_color=COLOR_CARD_BORDER, line_width=Pt(1))

    leader_header = slide.shapes.add_shape(MSO_SHAPE.ROUNDED_RECTANGLE, left, y, width, leader_header_h)
    leader_header.adjustments[0] = 0.5
    ...
    member_card = slide.shapes.add_shape(MSO_SHAPE.ROUNDED_RECTANGLE, left, y, width, member_card_h)
    member_card.adjustments[0] = 0.06
    _set_shape_fill(member_card, COLOR_CARD_BG, line_color=COLOR_CARD_BORDER, line_width=Pt(1))

    member_header = slide.shapes.add_shape(MSO_SHAPE.ROUNDED_RECTANGLE, left, y, width, member_header_h)
    member_header.adjustments[0] = 0.5
